...
function Comments({ noteId }) {
  ...
  const fetchComments = async (filter) => {
    ...
    try {
      const response = await axios.get(
        `${Base_url}/display_comment/${noteId}?filter=${filter}`
      );
  ...
  const handleReplyClick = async (commentId) => {
    ...
    if (commentId !== isReplying) {
      try {
        const response = await axios.get(
          `${Base_url}/display_comment_replay/${commentId}`
        );
  ...
  const handlePostComment = async () => {
    ...
    try {
      if (!isReplying) {
        ...
        const response = await axios.post(
          `${Base_url}/post_comment_replay/${isReplying}`,
          { comment: newComment }
        );
        setReplies((prevReplies) => ({
          ...prevReplies,
  ...
  return (
    <>
      ...
      <div
        ...
        id={`CommentBox-${noteId}`}
        tabIndex="-1"
        aria-labelledby={`CommentBoxLabel-${noteId}`}
        aria-hidden="true"
      >
        <div className="modal-dialog modal-dialog-scrollable modal-dialog-centered modal-lg">
          <div className="modal-content">
            <div className="modal-header">
              <h5 className="modal-title" id={`CommentBoxLabel-${noteId}`}>
                Comment Box
              </h5>
              <select
                className="commandfilter"
                      ...
                      <div className="card mt-2">
                        ...
                        <div className="card-body">
                          ...
                          <div className="card-footer">
                            ...
                            <p className="d-inline p-2 counters">
                ...
                <div className="row">
                  <div className="col d-flex justify-content-end">
                    <input
                      ...
                      type="text"
                      placeholder={
                        isReplying ? `Reply...@${isReplying}` : "Comment..."
                      }
                      value={newComment}
